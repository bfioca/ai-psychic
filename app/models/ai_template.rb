...
class AiTemplate
  ...
  def run(params:)
    ...
    if Rails.application.credentials.open_ai_secret_key.nil?
  ...
  def self.load(token:)
    ...
    if !File.exist?(Dir.pwd + "/app/models/ai_templates/#{token}.txt")
      raise "No template found for token #{token}"
    end
    text = File.read(Dir.pwd + "/app/models/ai_templates/#{token}.txt")
